...
public class SearchByAhoStrategy extends ASearchStrategy  {
	...
	public int searchFile(String content, String word) {
		SearchByAhoStrategy sm = new SearchByAhoStrategy();
		String[] input = {word};
		sm.createTrie(input);
		sm.getFailure();
		LinkedList<String> matches = sm.search(content,true);
		
		int ret = matches.size();
		return ret;
